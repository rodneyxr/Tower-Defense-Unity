...
public class UnitTower : Unit {
		...
		while(true){
			if(built && !stunned){
				if(target==null){
					...
					if(currentTargetDist>range || target.HPAttribute.HP<=0 || !target.thisObj.activeSelf){
						target=null;
					}
				}
			}
			...
			if(built && !stunned){
				...
				if(cols.Length>0){
					...
					if(mineOneOff){
						...
						if(thisT.childCount>0) thisObj.SetActive(false);
	...
	public void HitTarget(Vector3 pos, Unit tgt, bool effect, int div){
		if(aoeRadius<=0){
			if(tgt.gameObject!=null && tgt.gameObject.activeSelf){
	...
	private void UpgradeStat(){
		...
		if(upgradeStat[levelM].turretObject!=null){
			...
			if(turretObject.childCount>0) turretObject.gameObject.SetActive(false);
            else turretObject.gameObject.SetActive(false);
		...
		if(upgradeStat[levelM].baseObject!=null){
			...
			if(baseObject.childCount>0) baseObject.gameObject.SetActive(false);
            else baseObject.gameObject.SetActive(false);
